...
console.log(
  `   Root: ${rootEnvPath} (${fs.existsSync(rootEnvPath) ? "✅" : "❌"})`,
);
console.log(
  `   API: ${apiEnvPath} (${fs.existsSync(apiEnvPath) ? "✅" : "❌"})`,
);
console.log(
  `   Workspace: ${workspaceRoot} (${fs.existsSync(workspaceRoot) ? "✅" : "❌"})`,
);
...
async function testPackageImageUpload() {
  ...
  try {
    ...
    console.log(
      `   ✅ URL is Cloudinary: ${result.url.includes("cloudinary.com") ? "Yes" : "No"}\n`,
    );
...
// Test image deletion (unused but kept for future use)
async function _testImageDeletion(publicId: string) {
...
async function testSignedUrl(publicId: string) {
  ...
  try {
    ...
    console.log(
      `   ✅ URL is Cloudinary: ${signedUrl.includes("cloudinary.com") ? "Yes" : "No"}\n`,
    );
...
async function runTests() {
  ...
  if (!cloudName || !apiKey || !apiSecret) {
    console.log(
      "❌ Cloudinary is not configured. Please set the required environment variables.\n",
    );
    process.exit(1);
  }
  ...
  const _deletionResult = { success: false };
  ...
  console.log(
    `   Package Image Upload: ${results.packageUpload.success ? "✅" : "❌"}`,
  );
  console.log(
    `   Profile Picture Upload: ${results.profileUpload.success ? "✅" : "❌"}`,
  );
  console.log(
    `   Generic File Upload: ${results.genericUpload.success ? "✅" : "❌"}`,
  );
  console.log(
    `   Signed URL Generation: ${signedUrlResult.success ? "✅" : "❌"}`,
  );
  console.log(`   Image Deletion: ⚠️  Skipped (to keep test images)\n`);

  const allPassed =
    results.packageUpload.success &&
