  ...
  try {
    ...
    if (webhookSecret && webhookSecret !== 'hsk_your-paystack-webhook-secret') {
      if (!signature) {
        ...
        const isValid = verifyPaystackSignature(payload, signature, webhookSecret);
        ...
        if (!isValid) {
          ...
          console.warn('⚠️  Development mode: Continuing despite invalid signature');
        }
      }
    } else {
      ...
      console.warn('⚠️  Development mode: Webhook secret not configured. Signature verification skipped.');
    }
...
async function handleRefundProcessed(data: any, prisma: any) {
  try {
    ...
    console.log(`Processing refund: ${transactionReference}, Amount: ${amount}`);
...
router.post('/stripe', async (req: Request, res: Response) => {
  try {
    ...
    res.status(200).json({ received: true, message: 'Stripe webhook handler not yet implemented' });
  } catch (error: any) {
