...
export class FacialRecognitionService {
  ...
  async performFacialRecognition(
    ...
    try {
      ...
      const livenessResult =
        await this.googleVision.analyzeFaceLiveness(selfieImageBase64);
      ...
      throw new Error(
        `Facial recognition failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
  ...
  async analyzeDocumentFaceQuality(imageBase64: string): Promise<{
    ...
    try {
      const _imageBuffer = Buffer.from(imageBase64, "base64");
      ...
      if (!result.faceDetected) {
        ...
        recommendations.push(
          "Ensure face is clearly visible in document photo",
        );
      }
      ...
      if (result.landmarks.length < 5) {
        ...
        recommendations.push(
          "Ensure face is positioned correctly and clearly visible",
        );
      }
  ...
  async analyzeSelfieQuality(imageBase64: string): Promise<{
    ...
    try {
      ...
      const livenessResult =
        await this.googleVision.analyzeFaceLiveness(imageBase64);
      ...
      if (!livenessResult.isLive) {
        ...
        recommendations.push(
          "Ensure you are taking a live selfie, not a photo of a photo",
        );
      }
  ...
  async compareMultipleFaces(
    ...
    try {
      ...
      if (consistency < 0.7) {
        recommendations.push(
          "Face consistency is low - consider retaking photos",
        );
      }
      ...
      switch (documentType) {
        case "DRIVERS_LICENSE":
          if (qualityResult.quality < 0.7) {
            ...
            recommendations.push(
              "Ensure clear, well-lit photo of driver license",
            );
          }
          break;
