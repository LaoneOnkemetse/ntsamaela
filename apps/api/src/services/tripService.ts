...
class TripService {
  ...
  async createTrip(tripData: CreateTripRequest): Promise<Trip> {
    try {
      ...
      if (!driver.user?.identityVerified) {
        throw new AppError(
          "Driver must be verified to create trips",
          "DRIVER_NOT_VERIFIED",
          403,
        );
      }
  ...
  async updateTrip(
    ...
    try {
      ...
      if (driverId && existingTrip.driverId !== driverId) {
        throw new AppError(
          "Unauthorized to update this trip",
          "UNAUTHORIZED",
          403,
        );
      }
      ...
      const trip = await this.getPrisma().trip.update({
        ...
        data: {
          ...(updateData.startAddress && {
            startAddress: updateData.startAddress,
          }),
          ...
          ...(updateData.availableCapacity && {
            availableCapacity: updateData.availableCapacity,
          }),
  ...
  async deleteTrip(id: string, driverId?: string): Promise<void> {
    try {
      ...
      if (driverId && existingTrip.driverId !== driverId) {
        throw new AppError(
          "Unauthorized to delete this trip",
          "UNAUTHORIZED",
          403,
        );
      }
      ...
      const hasAcceptedBids = existingTrip.bids?.some(
        (bid: any) => bid.status === "ACCEPTED",
      );
      if (hasAcceptedBids) {
        throw new AppError(
          "Cannot delete trip with accepted bids",
          "TRIP_HAS_ACCEPTED_BIDS",
          400,
        );
      }
