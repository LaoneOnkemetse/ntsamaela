...
export class MockRealtimeService {
  ...
  private setupEventHandlers() {
    ...
    this.io.on("connection", (socket) => {
      ...
      socket.on(
        "bid:accepted",
        (data: { packageId: string; bidId: string }) => {
          this.io?.emit("bid:accepted", data);
          console.log(
            `Mock: Bid ${data.bidId} accepted for package ${data.packageId}`,
          );
        },
      );
      ...
      socket.on(
        ...
        (data: { packageId: string; bidId: string }) => {
          ...
          console.log(
            `Mock: Bid ${data.bidId} rejected for package ${data.packageId}`,
          );
        },
      );
      ...
      socket.on(
        ...
        (data: { packageId: string; status: string; tracking: any }) => {
          this.io?.emit("package:status:update", data);
          console.log(
            `Mock: Package ${data.packageId} status updated to ${data.status}`,
          );
        },
      );
      ...
      socket.on(
        ...
        (data: { packageId: string; latitude: number; longitude: number }) => {
      ...
      socket.on(
        "delivery:started",
        (data: { packageId: string; driverId: string }) => {
          this.io?.emit("delivery:started", data);
          console.log(
            `Mock: Delivery started for package ${data.packageId} by driver ${data.driverId}`,
          );
        },
      );
      ...
      socket.on(
        ...
        (data: { packageId: string; driverId: string }) => {
          ...
          console.log(
            `Mock: Delivery completed for package ${data.packageId} by driver ${data.driverId}`,
          );
        },
      );
      ...
      socket.on(
        "delivery:failed",
        (data: { packageId: string; driverId: string; reason: string }) => {
          this.io?.emit("delivery:failed", data);
          console.log(
            `Mock: Delivery failed for package ${data.packageId}: ${data.reason}`,
          );
        },
      );
  ...
  async getChatMessages(
    ...
    console.log(
      `Mock: Retrieved ${messages.length} messages for chat room ${chatRoomId}`,
    );
    return messages;
  }
  ...
  async getUserNotifications(
    ...
    console.log(
      `Mock: Retrieved ${notifications.length} notifications for user ${userId}`,
    );
