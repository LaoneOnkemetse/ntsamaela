...
export default function Dashboard() {
  ...
  useEffect(() => {
    const status = (
      dashboardError as unknown as { response?: { status?: number } }
    )?.response?.status;
  ...
  type PackageRaw = {
    ...
    driver?: { firstName?: string; lastName?: string } | string;
  };

  const packagesList = Array.isArray(packagesData) ? packagesData : [];
  const recentPackages: PackageItem[] = packagesList.map((pkg: PackageRaw) => {
    const cust =
      pkg.customer && typeof pkg.customer === "object" && pkg.customer.firstName
        ? `${pkg.customer.firstName} ${pkg.customer.lastName ?? ""}`.trim()
        : pkg.customerName || "Unknown";
    const drv =
      pkg.driver && typeof pkg.driver === "object" && pkg.driver.firstName
        ? `${pkg.driver.firstName} ${pkg.driver.lastName ?? ""}`.trim()
    ...
    return {
      ...
      status: (pkg.status || "pending").toLowerCase(),
    };
  });

  const verificationsList = Array.isArray(verificationsData)
    ? verificationsData
    : [];
                    ...
                    {packagesLoading ? (
                      ...
                      recentPackages.map((pkg: PackageItem) => {
                        ...
                        return (
                          ...
                          >
                            <TableCell
                              sx={{ fontWeight: 600, color: "#75AADB" }}
                            >
                              {pkg.id}
                            </TableCell>
                  ...
                  >
                    {[
                      ...
                      },
                      {
                        ...
                        count:
                          packagesList.filter(
                            (p: PackageItem) => p.status === "PENDING",
                          ).length || 0,
                        color: "#FF6D00",
                        action: "approvals",
                      },
                      {
                        ...
                        count:
                          dashboardData?.activePackages ||
                          packagesList.filter(
                            (p: PackageItem) => p.status === "IN_TRANSIT",
                          ).length ||
                          0,
                        color: "#75AADB",
                        action: "support",
