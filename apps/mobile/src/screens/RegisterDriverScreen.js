      ...
      <SafeAreaView style={{ flex: 1 }}>
        ...
        <ScrollView style={sharedStyles.formContainer} contentContainerStyle={{ paddingBottom: 120 }}>
          ...
          <View style={sharedStyles.documentsSection}>
            ...
            <Text style={sharedStyles.documentsSubtitle}>Selfie and Driver's License</Text>
            <RegistrationPhotoButton label="Selfie *" onPress={handleTakeSelfie} preview={selfie} />
            <RegistrationPhotoButton 
              label="License Front *" 
              onPress={() => showPhotoActionSheet(
                async () => { const img = await takePhoto(); if (img) setDriversLicenseFront(img); },
                async () => { const img = await selectFromGallery(); if (img) setDriversLicenseFront(img); }
              )} 
            ...
            <RegistrationPhotoButton 
              label="License Back *" 
              onPress={() => showPhotoActionSheet(
                async () => { const img = await takePhoto(); if (img) setDriversLicenseBack(img); },
                async () => { const img = await selectFromGallery(); if (img) setDriversLicenseBack(img); }
              )} 
          ...
          <View style={sharedStyles.documentsSection}>
            ...
            <Text style={sharedStyles.documentsSubtitle}>Car registration and description</Text>
            ...
            <RegistrationPhotoButton 
              ...
              onPress={() => showPhotoActionSheet(
                async () => { const img = await takePhoto(); if (img) setCarPhoto(img); },
                async () => { const img = await selectFromGallery(); if (img) setCarPhoto(img); }
              )} 
              preview={carPhoto} 
            />
          </View>
